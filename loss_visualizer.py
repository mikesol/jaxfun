...
(playhead,) = ax.plot(
    ...
    [np.min(input_wave), np.max(target_wave)],
    color="green",
    linewidth=2,
    label="Playhead",
)
...
def zoom(factor, axis):
    if axis == "x":
        ...
        ax.set_xlim(
            [center - (center - left) / factor, center + (right - center) / factor]
        )
    elif axis == "y":
        bottom, top = ax.get_ylim()
        center = (bottom + top) / 2
        ax.set_ylim(
            [center - (center - bottom) / factor, center + (top - center) / factor]
        )
    fig.canvas.draw_idle()
...
sample_slider = Slider(
    axsamples, "Sample", 0, len(input_wave) - 1, valinit=0, valstep=1
)
