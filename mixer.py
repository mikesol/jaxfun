...
class MultiBiquadWithSidechain(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        # (b, c, k, s)
        inputs = jnp.reshape(inputs, (batch_size, self.channels, 3, -1))
        # (b, c, s, k)
        inputs = jnp.transpose(inputs, (0, 3, 1, 2))
        return jnp.squeeze(
            nn.RNN(
                MultiBiquadCellWithSidechain(
                    channels=self.channels, carry_init=self.carry_init
                )
            )(inputs),
            axis=2,
        )
...
if __name__ == "__main__":
    ...
    channels = 2**10
    model = MixingBoard(channels=channels, depth=8)
